...
const ZOBRIST_PIECES = Array.from({ length: BOARD_SIZE * BOARD_SIZE }, () => [
...
const ZOBRIST_PENDING = Array.from({ length: BOARD_SIZE * BOARD_SIZE }, () => random64(zobristRand32));
...
function updateSeoMetadata() {
    ...
    const jsonLdNode = document.getElementById("ldJsonApp");
    if (jsonLdNode) {
        const jsonLd = {
...
const elements = {
    layout: getRequiredElement("layout"),
    editorPanel: getRequiredElement("editorPanel"),
    settingsPanel: getRequiredElement("settingsPanel"),
    board: getRequiredElement("board"),
    status: getRequiredElement("status"),
    hintText: getRequiredElement("hintText"),
    modeGroup: getRequiredElement("modeGroup"),
    variantGroup: getRequiredElement("variantGroup"),
    colorGroup: getRequiredElement("colorGroup"),
    langGroup: getRequiredElement("langGroup"),
    settingsToggleBtn: getRequiredElement("settingsToggleBtn"),
    leaderboardPanel: getRequiredElement("leaderboardPanel"),
    leaderboardBody: getRequiredElement("leaderboardBody"),
    leaderboardEmpty: getRequiredElement("leaderboardEmpty"),
    langRuBtn: getRequiredElement("langRuBtn"),
    langEnBtn: getRequiredElement("langEnBtn"),
    modeAiBtn: getRequiredElement("modeAiBtn"),
    modePvpBtn: getRequiredElement("modePvpBtn"),
    variantClassicBtn: getRequiredElement("variantClassicBtn"),
    variantGiveawayBtn: getRequiredElement("variantGiveawayBtn"),
    colorWhiteBtn: getRequiredElement("colorWhiteBtn"),
    colorBlackBtn: getRequiredElement("colorBlackBtn"),
    whiteTimer: getRequiredElement("whiteTimer"),
    blackTimer: getRequiredElement("blackTimer"),
    timeControl: getRequiredElement("timeControl"),
    difficulty: getRequiredElement("difficulty"),
    newGameBtn: getRequiredElement("newGameBtn"),
    undoBtn: getRequiredElement("undoBtn"),
    hintOnceBtn: getRequiredElement("hintOnceBtn"),
    hintBtn: getRequiredElement("hintBtn"),
    enterSetupBtn: getRequiredElement("enterSetupBtn"),
    setupPanel: getRequiredElement("setupPanel"),
    setupPiece: getRequiredElement("setupPiece"),
    setupTurn: getRequiredElement("setupTurn"),
    setupHumanColor: getRequiredElement("setupHumanColor"),
    clearBoardBtn: getRequiredElement("clearBoardBtn"),
...
let rankedSessionToken = null;
function isAiMode() {
    return state.gameMode === "ai";
}
function isGiveawayVariant() {
    return state.variant === "giveaway";
}
function winnerWhenNoLegalMoves(turn) {
...
async function refreshLeaderboardFromServer() {
    const data = await apiFetchJson(`/leaderboard?limit=${LEADERBOARD_LIMIT}`);
...
function isLeaderboardQualifyingScore(score) {
...
function beginGameSession(startedFromSetup = false) {
    ...
    void apiFetchJson("/session/start", {
        method: "POST",
        body: JSON.stringify({
            mode: state.gameMode,
            difficulty: state.difficulty,
...
function maybeFinalizeLeaderboardForAi(reason) {
    ...
    const result = state.winner === state.humanColor ? "win" : "loss";
...
function squareLabel(r, c, piece) {
...
function getBoardToDisplayCoords(r, c) {
    if (state.humanColor === "w") {
        return { r, c };
    }
...
function appendMoves(target, source) {
...
function applyMoveMutable(node, move) {
...
function undoMoveMutable(node, undo) {
    const board = node.board;
...
function orderMoves(board, moves, preferredMove) {
...
function terminalScore(sideToMove, perspectiveColor, depth) {
...
function isPotentialVictim(board, r, c, victimColor) {
...
function evaluateGiveawayPosition(node, perspectiveColor, legalMoves) {
    let myMen = 0;
    let myKings = 0;
    let oppMen = 0;
    let oppKings = 0;
...
function evaluatePosition(node, perspectiveColor, legalMoves = null) {
    ...
    for (let r = 0; r < BOARD_SIZE; r += 1) {
        for (let c = 0; c < BOARD_SIZE; c += 1) {
            ...
            if ((isInside(supportRow, c - 1) && node.board[supportRow][c - 1]?.color === piece.color)
...
function checkSearchDeadline(deadline, clock) {
    clock.nodes += 1;
    if ((clock.nodes & 63) !== 0) {
        return;
    }
...
function minimax(node, depth, alpha, beta, rootColor, deadline, table, clock) {
...
function chooseBestMove(side, mode) {
    ...
    let depthReached = 0;
    let scoredAtDepth = [{ move: bestMove, score: bestScore }];
...
function applyBottomColor(color) {
...
function onSquareClick(r, c) {
    ...
    if (state.selected) {
        const selectedMoves = allMoves.filter((move) => move.from.r === state.selected.r && move.from.c === state.selected.c);
...
function applySetupAndPlay() {
    ...
    state.humanColor = elements.setupHumanColor.value;
    state.aiColor = opposite(state.humanColor);
    state.setupMode = false;
    state.setupBackup = null;
    state.turn = elements.setupTurn.value;
    state.pendingCapture = null;
    state.selected = null;
    state.legalMovesForSelected = [];
    state.gameOver = false;
...
function renderTimers() {
    ...
    elements.whiteTimer.classList.toggle("low", state.whiteTimeMs <= 30000);
    elements.blackTimer.classList.toggle("low", state.blackTimeMs <= 30000);
}
...
function renderBoard() {
    ...
    const targetSet = new Set(state.legalMovesForSelected.map((move) => `${move.to.r},${move.to.c}`));
